...
;(function($){
    ...
    $.fn.calendarLite.draw = function(opts) {
        ...
        for (var j = 0; j < dates.length; j++) {
            ...
            line.push('<td' + cl + '><a rel="' + rel + '">' + date + '</a></td>');
        ...
        var dateForMe = new Date();
        var c = dateForMe.getMonth();
        var cc = dateForMe.setMonth(+1);
        cc = new Date(cc);
        cc = cc.getMonth();

        if(c == curMonth) {
            o.prevArrow = '';
        } else o.prevArrow = 'Previous';

        if(cc == curMonth) {
            o.nextArrow = '';
        } else o.nextArrow = 'Next';

        $this[0].innerHTML = '<span class="state" style="display:none;">' + curMonth + '.' + curYear + '</span><div class="monthName"><a href="#" class="next">' + o.nextArrow + '</a><a href="#" class="prev">' + o.prevArrow + '</a> ' + '<span>'+o.months[curMonth]+'</span>' + y + ' </div>';
        $this.append(table)
